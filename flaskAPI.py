...
@app.route('/col35/prisonstatistics', methods=['GET'])
def get_csv_json():
    base_path = os.getcwd()
    folder_path = "Cleaned"  # Update this to your folder path
    path = os.path.join(base_path,folder_path)
    print(path)
    csv_files = [f for f in os.listdir(path) if f.endswith('.csv')][:6]  # Get the  CSV files
    
    if len(csv_files) < 5:
        return jsonify({"error": "Less than 6 CSV files found"}), 400
...
@app.route('/col35/metadataapi', methods=['GET'])
def get_json_file():
